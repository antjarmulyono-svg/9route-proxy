...
async function installCertLinux(sudoPassword, certPath) {
  // Root already has the access sudo would grant — containers ship no sudo binary,
  // so gating on isSudoAvailable() alone silently skips the install and leaves the
  // dashboard's "Trusted" badge red with nothing written to the store.
  if (!isSudoAvailable() && !isRoot()) {
  ...
  const config = getLinuxCertConfig();
  if (!fs.existsSync(config.dir)) {
    // Alpine images have no ca-certificates package — create the anchor dir so the
    // copy below lands somewhere real instead of failing on a missing path.
    try { fs.mkdirSync(config.dir, { recursive: true }); } catch { /* handled by copy error */ }
  }
