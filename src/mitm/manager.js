...
const { LSOF_BIN, listeningPids, processNameForPid } = require("./config");
...
async function resolveMitmRouterBaseUrl() {
  // In container environments (no sudo), always connect to local loopback
  if (!isSudoAvailable()) return DEFAULT_MITM_ROUTER_BASE;
...
function getProcessUsingPort443() {
  try {
    if (IS_WIN) {
      ...
      const procPids = listeningPids(MITM_PORT);
      if (procPids) return procPids.length ? processNameForPid(procPids[0]) : null;
...
function isProcessAlive(pid) {
  try {
    ...
    // Signalling a process owned by another user throws EPERM (EACCES on some
    // platforms) — it exists, we just may not touch it. Only ESRCH means dead.
...
function getPort443Owner(sudoPassword) {
  return new Promise((resolve) => {
    if (IS_WIN) {
      ...
      const procPids = listeningPids(MITM_PORT);
      if (procPids) {
        const pid = procPids[0];
        return resolve(pid ? { pid, name: processNameForPid(pid) } : null);
      }
      // macOS — real lsof, and BSD ps understands -p/-o comm=
...
/**
 * Adopt a healthy MITM server this process did not spawn — one started with sudo that
 * outlived an app restart, or launched out-of-band. `serverProcess` is null for those
 * and an unprivileged app can neither signal nor pkill a root-owned PID, so without
 * adoption every start attempt reports a phantom port-443 conflict.
 * Returns the running descriptor when the port owner answers /_mitm_health, else null.
 */
async function adoptRunningMitm(ownerPid, sudoPassword) {
  const health = await pollMitmHealth(1000, MITM_PORT);
  if (!health) return null;
  const pid = health.pid || ownerPid || null;
  serverPid = pid;
  try { fs.writeFileSync(PID_FILE, String(pid)); } catch { /* best effort */ }
  try { fs.unlinkSync(LOCK_FILE); } catch { /* ignore */ }
  log(`♻️ Adopting healthy MITM already on port ${MITM_PORT} (PID: ${pid})`);
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (!IS_WIN) {
    ...
    if (portStatus === "in-use" || portStatus === "no-permission") {
      ...
      // Adoption is tried before the owner is even identified: procfs cannot see a
      // listener owned by another user unless we are root, so a healthy MITM started
      // with sudo shows up as an unidentifiable holder rather than as ours.
      if (!forceKillPort443) {
        const adopted = await adoptRunningMitm(owner?.pid, sudoPassword);
  ...
  if (IS_WIN) {
    ...
    if (winOwner) {
      if (forceKillPort443) {
        ...
        const adopted = await adoptRunningMitm(winOwner.pid, sudoPassword);
        if (adopted) return adopted;
